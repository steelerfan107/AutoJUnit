...
public class DAGNode {
    ...
    public void addChild(DAGNode child) {
        if (children == null) {
            children = new DAGNode[1];
            children[0] = child;
        } else {
            Random r = new Random();
            if (r.nextInt(2) == 0) {
                DAGNode[] newChildren = new DAGNode[children.length + 1];
                System.arraycopy(children, 0, newChildren, 0, children.length);
                newChildren[newChildren.length - 1] = child;
                children = newChildren;
            } else {
                DAGNode randomChild = children[r.nextInt(children.length)];
                randomChild.addChild(child);
            }
        }
